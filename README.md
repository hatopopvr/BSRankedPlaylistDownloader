...
[Beat Saber用] Apluluさん(@aplulu_cat)作成のranked_all.zip をダウンロード・解凍し、
playlistフォルダに展開・上書きするスクリプトです。
...
- 設定
BeatSaberのPlaylistsフォルダを `config.ini`の`param`、`playlist_dir`に設定します。
